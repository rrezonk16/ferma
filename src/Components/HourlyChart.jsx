...
export default function HourlyChart() {
  ...
  useEffect(() => {
    ...
    axios
    .get("https://9738-46-99-13-247.ngrok-free.app/api/sensor/temperature/hourly", {
      headers: {
      ...
      <div className="max-w-screen-xl flex-wrap items-center justify-between mx-auto flex flex-col mt-10 bg-white p-6 rounded-lg shadow-md">
        ...
        <LineChart
          xAxis={[
            {
              scaleType: "band",
              data: temperatureData.map((item) => item.hour).reverse(),
            },
          ]}
          series={[
            {
              data: temperatureData.map((item) => item.temperature).reverse(),
            },
          ]}
          height={350}
